"""Реализация KnowledgeBase — локальная нормативная база."""
...
class LocalKnowledgeBase:
    ...
    def _load(self) -> None:
        ...
        # 5. Диагностика целостности для админки
    ...
    def list_subjects(self, grade: int) -> list[Subject]:
        """Возвращает только те предметы для класса, по которым есть хотя бы один результат в каталоге."""
        try:
    ...
    def check_topic(self, grade: int, subject_id: str, topic: str) -> TopicCheck:
        ...
        if len(self.index.chunks) > 0:
            ...
            # Фильтруем результаты: тема должна покрывать >= 60% значимых слов
            hits = [h for h in raw_hits if topic_coverage(topic, h.chunk.text) >= 0.60]

        # 2. Поиск по формулировкам результатов каталога этого класса и предмета
        ...
        # Проверяем, есть ли тема в результатах этого класса из каталога
        ...
        # Проверяем, есть ли тема в результатах других классов этого же предмета
    ...
    def _extract_suggestions(self, grade: int, subject_id: str, target_outcomes: list[Outcome]) -> list[str]:
        ...
        # Если подсказок меньше 5 (или нет content_chunks), берём из каталога результатов
    ...
    async def update(self, progress: ProgressCallback | None = None) -> UpdateReport:
        """Полный цикл обновления: скачивание -> парсинг -> нарезка -> каталог -> индекс."""
        ...
        # 3. Нарезка на фрагменты (чанкинг)
        chunks_dir = self.settings.knowledge_dir / "chunks"
    ...
    async def ingest_school_material(
        ...
        if filename.lower().endswith(".docx"):
            # Извлечение из DOCX блоками по 3000 символов
            import docx
