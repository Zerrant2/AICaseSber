...
а зависит только от протоколов соседей — это позволяет трём агентам работать параллельно,
подставляя фейки из socrat.testing.fakes.

  KnowledgeBase      — реализует knowledge  (Gemini)
  LLMClient          — реализует llm        (Claude)
  WorkGenerator      — реализует core       (Claude)
  ErrorAnalyzer      — реализует core       (Claude)
  ResponseObserver   — реализует core       (Claude)
  Exporter           — реализует export     (Codex)
  *Repository        — реализует storage    (Codex)
  Services           — контейнер, собирается в socrat.app (Codex)
"""
