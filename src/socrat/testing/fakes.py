"""Фейковые реализации всех протоколов — для параллельной разработки и тестов.
  ...
  * Codex строит бот/экспорт/хранилище на FakeWorkGenerator/FakeErrorAnalyzer/FakeKnowledgeBase,
    не дожидаясь реального core и knowledge. Включается USE_FAKES=true в .env.
  * Claude тестирует core на FakeKnowledgeBase (данные кейса SK01) и FakeLLMClient.
  * Gemini сверяет свою KnowledgeBase с поведением FakeKnowledgeBase (те же сигнатуры).

ВЛАДЕЛЕЦ: Claude. Реальные модули не должны импортировать отсюда ничего, кроме тестов/app.py.
"""
...
class FakeResponseObserver:
    """Простая эвристика. Реальная версия — core.observation (Claude)."""
