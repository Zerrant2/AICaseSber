"""Тесты класса LocalKnowledgeBase."""
...
def test_kb(tmp_path: Path) -> LocalKnowledgeBase:
    """Изолированная база знаний в tmp_path, не пишущая в реальный data/knowledge/."""
...
def test_list_subjects_ooo(test_kb: LocalKnowledgeBase):
    """Проверка доступности только предметов с каталогами."""
...
def test_check_topic_g14(test_kb: LocalKnowledgeBase):
    """Проверка: 5 типичных тем для algebra 7, russian 5, physics 8, и отсечение Фотосинтеза."""
...
def test_check_topic_g16_cross_grade_rejection(test_kb: LocalKnowledgeBase):
    """Проверка: отсечение тем чужого класса и тем не из программы."""
    negatives = [
...
def test_check_topic_g17_different_grade_topics(test_kb: LocalKnowledgeBase):
    """Проверка: темы из программы предмета, но другого класса (алгебра 7/8, геометрия 7/9)."""
...
def test_check_topic_fallback_without_index_or_pages(tmp_path: Path):
    """Проверка работы check_topic на чистом клоне без index/ и pages/."""
    real_settings = get_settings()
    k_dir = tmp_path / "knowledge"
    k_dir.mkdir(parents=True)
...
def test_status_diagnostics_reports_missing_pages_or_catalog(tmp_path: Path):
    """Проверка диагностики в status().problems для админки."""
    real_settings = get_settings()
    k_dir = tmp_path / "knowledge"
    k_dir.mkdir(parents=True)
